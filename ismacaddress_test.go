...
func TestIsMacAddress(t *testing.T) {
	tests := []struct {
		...
		{name: "Correct no separator", param1: "001A2B3C4D5E", param2: &IsMacAddressOpts{NoSeparators: Bool(true)}, want: true},
		// Invalid Mac address NoSeparators true
		{name: "Incorrect no separator", param1: "00:1A:2B:3C:4D:5E", param2: &IsMacAddressOpts{NoSeparators: Bool(true)}, want: false},
		{name: "Incorrect wrong types", param1: "00:1A:2B:3C:4D:5E", param2: &IsMacAddressOpts{NoSeparators: Bool(true), Type: strPtr("56")}, want: false},
		...
		{name: "Correct no separator EUI 64", param1: "001A2B3C4D5E6F70", param2: &IsMacAddressOpts{NoSeparators: Bool(true), Type: strPtr("64")}, want: true},
		// Invalid Mac address EUI 64, NoSeparators true
		{name: "Incorrect no separator  EUI 64", param1: "00-1A-2B-3C-4D-5E-6F-70", param2: &IsMacAddressOpts{NoSeparators: Bool(true), Type: strPtr("64")}, want: false},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result, err := IsMacAddress(test.param1, test.param2)
