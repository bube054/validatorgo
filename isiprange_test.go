...
func TestIsIPrange(t *testing.T) {
	tests := []struct {
		...
		{name: "Valid range 4 but no version specified", param1: "192.168.0.0/24", param2: "", want: true},
		// Invalid IPv4 Ranges
		...
		{name: "Incorrect range", param1: "10.0.0.1 - 10.0.0", param2: "8", want: false},
		// Valid IPv6 Ranges
		...
		{name: "Valid range 6 but no version specified", param1: "2001:0db8::/32", param2: "", want: true},
		// Invalid IPv6 Ranges
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result, err := IsIPRange(test.param1, test.param2)
